...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      hr(), 
      # Unit Costs - IDs align with the Server's u_costs list
      ...
      tabItem(tabName = "dash",
              ...
              fluidRow(
                box(title = "1) Most Cost-Effective Plan Changes", width = 6, solidHeader = TRUE, status = "success", 
                    p("Displays the geographic shift from the baseline to the strategy maximizing economic value (Net Monetary Benefit).",
                ...
                box(title = "2) Optimal Assessment Changes", width = 6, solidHeader = TRUE, status = "primary", 
                    p("Identifies where to reallocate resources to maximize health outcomes within the current budget constraints.", 
                      style = "font-size: 13px; color: #666; font-style: italic;"),
